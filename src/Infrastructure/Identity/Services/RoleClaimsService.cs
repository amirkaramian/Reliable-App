...
public class RoleClaimsService : IRoleClaimsService
{
    ...
    public async Task<bool> RoleHasPermissionAsync(List<string> apiRoles, string apiKey, string permission)
    {
        var roles = new List<RoleDto>();
        string cacheKey = CacheKeys.GetCacheKey(ClaimConstants.Permission, apiKey);
        ...
        {
            var applicationRoles = await _db.Roles.Where(a => apiRoles.Contains(a.Name)).ToListAsync();
    ...
    public async Task<List<string>> GetAllUsersHasPermissions(string permission)
    {
        var adminRole = await _db.Roles.FirstOrDefaultAsync(x => x.Name.ToLower() == "admin");
        var allowedPermissionsRoleId = await _db.RoleClaims.Where(a => a.ClaimType == ClaimConstants.Permission && a.ClaimValue == permission && a.RoleId == adminRole.Id).Select(x => x.RoleId).ToListAsync();
        return await _db.UserRoles.Where(x => allowedPermissionsRoleId.Contains(x.RoleId)).Select(x => x.UserId).Distinct().ToListAsync();
    }
