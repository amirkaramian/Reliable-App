...
namespace MyReliableSite.Infrastructure.Persistence.Repositories;
...
public class RepositoryAsync : IRepositoryAsync
{
        ...
        where T : BaseEntity
    {
        ...
        if (_dbContext.Entry(entity).State == EntityState.Unchanged && _dbContext.Entry(entity).Entity.DomainEvents.Count == 0)
        {
    ...
    public Task ClearCacheAsync<T>(T entity)
       where T : BaseEntity
    {
        _cache.Remove(CacheKeys.GetCacheKey<T>(entity.Id));
        return Task.CompletedTask;
    ...
    public async Task<IReadOnlyList<T>> QueryWithDtoAsync<T>(string sql, object param = null, IDbTransaction transaction = null, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<T> LastByConditionAsync<T>(Expression<Func<T, bool>> expression, string[] orderBy = null, bool AsNoTracking = true, BaseSpecification<T> specification = null)
        where T : BaseEntity
    {
        var query = _dbContext.Set<T>().AsQueryable();
        if (AsNoTracking)
        ...
        query = query.ApplySort(orderBy, OrderTypeEnum.OrderByAscending);
        ...
        where TDto : IDto
    {
        ...
        query = query.ApplySort(orderBy, orderTypeEnum);
