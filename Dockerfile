...
COPY ["src/Admin/Admin.csproj", "src/Admin/"]
...
COPY ["/dotnet.ruleset", "src/Client/"]
COPY ["/dotnet.ruleset", "src/Admin/"]
...
COPY ["/stylecop.json", "src/Client/"]
COPY ["/stylecop.json", "src/Admin/"]
...
RUN dotnet restore "src/Admin/Admin.csproj" --disable-parallel
COPY . .
WORKDIR "/src/Admin"
...
WORKDIR /app/Files
WORKDIR /app
ENTRYPOINT ["dotnet", "MyReliableSite.Admin.dll"]
